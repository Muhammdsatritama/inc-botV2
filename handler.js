  ...
  async handler(chatUpdate) {
    ...
    try {
      ...
      try {
        ...
        if (settings) {
          if (!'jadibot' in settings) settings.gconly = false
      ...
      let isBlocked = this.blocklist.map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').filter(v => v != this.user.jid).includes(m.sender) // User Is Blocked?
    	for (let name in global.plugins) {
    	  let plugin = global.plugins[name]
        if (!plugin) continue
        if (plugin.disabled) continue
        if (!opts['restrict']) if (plugin.tags && plugin.tags.includes('admin')) continue
        const str2Regex = str => str.replace(/!#?,.$&+-_'%=^ZT/g, '\\$&')
        ...
        if (typeof plugin !== 'function') continue
          ...
          if (plugin.coin && global.db.data.users[m.sender].coin < plugin.coin * 1) {
            this.reply(m.chat, `Coin kamu habis, silahkan bekerja melalui *${usedPrefix}work*`, m)
            continue // Coin habis
          }
          if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 1) {
            this.reply(m.chat, `Limit kamu habis, silahkan beli melalui *${usedPrefix}shop*`, m)
          ...
          try {
            ...
            if (m.limit) m.reply(+ m.limit + ' *Limit terpakai*\nGunakan limit seperlunya, agar tidak habis dengan sia-sia')
          }
    			break
  	  	}
    	}
...
「 *DELETE CHAT DETECTED* 」

Terdeteksi @${m.participant.split`@`[0]} telah menghapus pesan

Untuk mematikan fitur ini, silahkan ketik
*#disable <command>*
`.trim(), m.message, {
      contextInfo: {
  ...
  async onCall(json) {
    ...
    await this.sendMessage(from, 'Maaf, karena anda menelfon bot.\nNomor anda diblokir otomatis!', MessageType.extendedText)
...
global.dfail = (type, m, conn) => {
  let msg = {
    ...
    unreg: 'Kamu harus memverifikasi data terlebih dahulu! gunakan:\n\n*#daftar namamu.umurmu*\n\nContoh: #daftar Manusia.15'
  }[type]
