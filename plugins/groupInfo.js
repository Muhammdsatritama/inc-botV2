...
let handler = async (m, { conn, participants, groupMetadata }) => {
  await conn.updatePresence(m.chat, Presence.composing)
    ...
    let pp = './src/apatar.png'
    try {
        pp = await conn.getProfilePicture(m.chat)
    } catch (e) {
    } finally {
        let { isBanned, welcome, detect, sWelcome, sBye, sPromote, sDemote, antiLink, antiVirtex } = global.db.data.chats[m.chat]
