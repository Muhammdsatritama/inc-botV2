...
let handler  = async (m, { conn, args, usedPrefix, command }) => {
  if (!global.db.data.settings.jadibot) throw 'Jadi Bot\n\n- 1 hari Rp. 2.000,-\nhttps://github.com/Muhammdsatritama'
  ...
  if ((args[0] && args[0] == 'plz') || global.conn.user.jid == conn.user.jid) {
    ...
    conn.connect().then(async ({user}) => {
      ...
      if (auth) return
      await parent.sendMessage(user.jid, `_Kamu bisa login tanpa qr dengan pesan dibawah ini. untuk mendapatkan kode lengkapnya, silahkan kirim_ *${usedPrefix}getcode* _untuk mendapatkan kode yang akurat_`, MessageType.extendedText)
